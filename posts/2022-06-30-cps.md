...
+ **The code is deeply nested.** It's slowly turns into a weird staircase as
we add more steps.
+ **It mixes business logic with pesky implementation details.** The
  description of this function has only four clear steps but it's
  extremely difficult to view them quickly from the
  implementation. The problem becomes more severe as you have more
  steps or start introducing more complicated logic (e.g. logging,
  monitoring, etc.).

## Either-isation

A one possible approach to improving the situation would be to
introduce a separate function for each individual step. Thouse
...
understandable due to uniform handling of errors).
...
anything)`, you can use the `ExceptT` monad transformer.
...
After performing this refactoring to each function, we can update the
...
2. Added extra argument — continuation — a function of type
   ...
   continuation is polymorphic (as well as return type of our
...
with an error or runs a given action with `userId` in case of
...
This works because Haskell rules around parsing layouts are quite
...
blocks but surprisingly works for lambdas.
